...
public:
    explicit InvalidModelException(const std::filesystem::path& file, const std::string& reason)
        : Exception("Model \"" + file.generic_string() + "\" is invalid! " + reason) { }
};

class ShaderCompileException : public Exception {
public:
    explicit ShaderCompileException(
        const std::string& shaderType, const std::string& shaderName, const std::string& infoLog)
        : Exception(shaderType + " shader \"" + shaderName + "\" failed to compile. Info log: \n" + infoLog) { }
};
