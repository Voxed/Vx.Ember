...
std::shared_ptr<Node> GLTFLoader::getNode(int id) {
    if (!nodeCache.contains(id)) {
        ...
        if (gltfNode.mesh != -1) {
            node->addChild(std::make_shared<GeometryNode>(getMesh(gltfNode.mesh), _model.meshes[gltfNode.mesh].name));
        }
...
std::shared_ptr<CameraNode> GLTFLoader::getCamera(int id) {
    if (!cameraCache.contains(id)) {
        ...
        auto camera = std::make_shared<CameraNode>(perspective.yfov, perspective.zfar, perspective.znear, gltfCam.name);
    ...
    if (!meshCache.contains(id)) {
        ...
        for (auto &primitive: gltfMesh.primitives) {
            ...
            std::vector<unsigned int> fIndices;
            {
                ...
                fIndices = std::vector<unsigned int>((unsigned short *) &(*(buffer.data.begin() + view.byteOffset)),
            ...
            std::vector<glm::vec3> fVertices;
            {
                ...
                fVertices = std::vector<glm::vec3>((glm::vec3 *) &(*(buffer.data.begin() + view.byteOffset)),
            ...
            std::vector<glm::vec3> fNormals;
            {
                ...
                fNormals = std::vector<glm::vec3>((glm::vec3 *) &(*(buffer.data.begin() + view.byteOffset)),
                                                  (glm::vec3 *) &(*(buffer.data.begin() + view.byteOffset +
                                                                    view.byteLength)));
            }
            auto primtive = std::make_shared<Primitive>(fVertices, fNormals, fIndices);
...
std::shared_ptr<Node> GLTFLoader::scene() {
    return _root;
}