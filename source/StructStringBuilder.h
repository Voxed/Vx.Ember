...
public:
    explicit StructStringBuilder(const char* name)
        : _name(name) {};

    // Objects
    template <typename T>
    requires(!std::is_arithmetic_v<T>) StructStringBuilder
        addField(const char* fieldName, T& fieldValue) {
    ...
    // Booleans
    template <typename T>
    requires(std::same_as<T, bool>) StructStringBuilder
        addField(const char* fieldName, T fieldValue) {
    ...
    template <typename T>
    requires(
        std::is_arithmetic_v<T> && !std::same_as<T, bool>) StructStringBuilder
        addField(const char* fieldName, T fieldValue) {
